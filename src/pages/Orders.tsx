  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-6" style={{ fontFamily: 'Playfair Display, serif' }}>Oda Zangu</h1>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl p-12 text-center">
            ...
            <p className="text-gray-500 mb-4">Bado huna oda yoyote</p>
            <Link to="/products"><Button className="bg-[#1a2332]">Anza Kununua</Button></Link>
          </div>
        ) : (
          <div className="space-y-4">
            {orders.map(order => (
              <div key={order.id} className="bg-white rounded-xl p-6 flex flex-col md:flex-row md:items-center gap-4 justify-between">
                <div>
                  <div className="text-xs text-gray-500">Oda #{order.id.substring(0, 8).toUpperCase()}</div>
                ...
                <div className="text-right">
                  ...
                  <Link to={`/order-confirmation/${order.id}`}>
                    <Button variant="outline" size="sm" className="mt-2">Tazama</Button>
                  </Link>
                </div>
              </div>
            ))}
